...
from func_robust_shadow import R_shadow_state_reconstruction, ket_to_rho
...
def state_tomography(total_photon, method):
    ...
    # robust SLST method
    if method == 'RSLST':
        shadow_ket = R_shadow_state_reconstruction(shadow, j)
    ...
    # MLE method
        ...
        infid_tomo = MLE_infid(shadow)
        return np.mean(1.0-infid_tomo)
...
if __name__ == '__main__':
    ...
    for exp_num in range(5):
        ...
        ######### robust SLST method
        infid_itr, state_para = state_tomography(total_photon, 'RSLST')
        ...
        ######### SLST method
        infid_itr, state_para = state_tomography(total_photon, 'SLST')
        infids_SLST.append(np.mean(infid_itr, axis=0)) # for plot
        fids_SLST.append(np.mean(1.0-infid_itr[:, -1]))
        state_paras_SLST.append(state_para)
        
        ######### MLE method
        MLE_fid = state_tomography(total_photon, method='MLE')
        MLE_fids.append(MLE_fid)
        
    print('RSLST fid: %.5f' %np.mean(fids_RSLST))
    print('SLST fid: %.5f' %np.mean(fids_SLST))
    ...
    # np.save('./result_data/noise_RSLST_iterative_prameters_state_'+str(state)+'_j='+str(j)+'.npy', state_paras_RSLST)

    # np.save('./result_data/noise_SLST_fidelity_state_'+str(state)+'_j='+str(j)+'.npy', 1.0-np.array(infids_SLST))
    # np.save('./result_data/noise_SLST_iterative_prameters_state_'+str(state)+'_j='+str(j)+'.npy', state_paras_SLST)
    
    # np.save('./result_data/noise_MLE_fidelity_state_'+str(state)+'_j='+str(j)+'.npy', MLE_fids)
    # print('Results saved')
