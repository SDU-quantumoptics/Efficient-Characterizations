...
if __name__ == '__main__':
    ...
    for exp_num in range(5):
        ...
        shadow_origin = np.load("./exp_shadow/itr_data/shadow_state"+str(state)+
                                "_M="+str(total_photon)+"_"+str(exp_num)+".npy")
    ...
    # np.save('./result_data(itr)/SLST_fidelity_state_'+str(state)+'_M='+str(total_photon)+'.npy', 1.0-np.array(infids))
    # np.save('./result_data(itr)/SLST_iterative_prameters_state_'+str(state)+'_M='+str(total_photon)+'.npy', state_paras)
    # np.save('./result_data(itr)/MLE_fidelity_state_'+str(state)+'_M='+str(total_photon)+'.npy', MLE_fids)
    print('Results saved')
    plot_infid(infids)
